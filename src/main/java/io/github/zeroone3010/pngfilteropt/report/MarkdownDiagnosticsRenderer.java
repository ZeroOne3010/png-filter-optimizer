...
public final class MarkdownDiagnosticsRenderer {
    ...
    public String render(List<Map<String, Object>> images) {
        return render(images, false, false);
    }

    public String render(List<Map<String, Object>> images, boolean verboseInsights) {
        return render(images, false, verboseInsights);
    }

    public String renderInsightsOnly(List<Map<String, Object>> images, boolean verbose) {
        return render(images, true, verbose);
    }

    private String render(List<Map<String, Object>> images, boolean insightsOnly, boolean verboseInsights) {
        StringBuilder sb = new StringBuilder(insightsOnly ? "\n## Compression insights\n\n" : "\n## Diagnostics\n\n");
        if (!insightsOnly) {
        ...
        for (Map<String, Object> image : images) {
            ...
            if (!insightsOnly) renderTables(sb, d);

            String best = (String) image.get("best");
            if (!insightsOnly) {
                sb.append("\nLikely explanation:\n")
                        .append(explanationGenerator.metricExplanation(best, d))
                        .append("\n\n");
            }
            sb.append("Compression insight:\n")
                    .append(explanationGenerator.compressionInsight(best, d, verboseInsights))
                    .append("\n\n");
        }
        return sb.toString();
    }

    private void renderTables(StringBuilder sb, Map<String, FilteredStreamDiagnostics> d) {
